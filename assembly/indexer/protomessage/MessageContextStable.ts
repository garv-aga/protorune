...
export class MessageContext {
  ...
  checkBalances(): bool {
    ...
    for (let i = 0; i < this.baseSheet.runes.length; i++) {
      if (
        ...
        checkingSheet.get(checkingSheet.runes[i])
      )
        return false;
    }
    return true;
  ...
  handleLiquidation(): void {
    for (let i = 0; i < this.runes.length; i++) {
      if (this.runes[i].liquidate()) {
        console.log("Liquidation triggered for rune: " + this.runes[i].runeId.toString());
      }
    }
  }

  handle(): void {
    // Call handleLiquidation to check for liquidation scenarios
